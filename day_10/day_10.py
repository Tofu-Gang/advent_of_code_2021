...
KEY_CLOSING_CHARACTER = "CLOSING_CHARACTER"
KEY_SCORE = "SCORE"
...
def syntax_error_score(line: str) -> int:
    ...
    character on the line and look it up in the syntax error scores table.
    """
    ...
    for char in line:
        if char in CHARACTERS_PAIRS.keys():
            # opening character
            stack.append(char)
        else:
            # closing character
            if char != CHARACTERS_PAIRS[stack.pop()]:
                # invalid closing character, get score and stop processing this
                # line
...
def autocomplete_score(line: str) -> int:
    """
    Incomplete lines are missing some closing characters at the end of the line.
    ...
    by the point value given for the character in the autocomplete scores table.
    """

    stack = []

    # figure out the incomplete chunks first
    for char in line:
        if char in CHARACTERS_PAIRS.keys():
            # opening character
            stack.append(char)
    ...
    # create the autocomplete string
    autocomplete_string = ""
    while len(stack) > 0:
        opening_char = stack.pop()
        autocomplete_string += CHARACTERS_PAIRS[opening_char]
...
def puzzle_2() -> None:
    ...
    with open(join(getcwd(), SELF_DIR_NAME, INPUT_TXT_NAME), "r") as f:
        lines = tuple([line.strip() for line in f.readlines()])
        incomplete_lines = tuple(filter(lambda line: syntax_error_score(line) == 0, lines))
        autocomplete_scores = tuple(sorted([autocomplete_score(line) for line in incomplete_lines]))

    # should be 2192104158
